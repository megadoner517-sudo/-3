...
<body>
    ...
    <!-- ЗАЩИТА ОТ ССЫЛКИ GITHUB НА СТРАНИЦЕ -->
    <style>
        /* Убираем все лишние элементы GitHub */
        body > :not(.card):not(.theme-switch):not(style):not(script):not(link) {
    ...
    <script>
        (function() {
            function cleanGarbage() {
                // Удаляем все ссылки с github
                var allLinks = document.querySelectorAll('a');
                for(var i = 0; i < allLinks.length; i++) {
                    var link = allLinks[i];
                    var href = link.getAttribute('href') || '';
                    if(href.indexOf('github') !== -1 || href.indexOf('github.io') !== -1) {
                        link.remove();
                    }
                }
